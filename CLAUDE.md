...
- Single output `HDMI-A-1` 1920×1080@144, scale 1. Webcam = Logitech C615 `/dev/video0` (720p/1080p). Default mic = the C615's mono capture. It sat at **53 % (hardware gain 6/18, +15 dB)** until 2026-09-03 — most of the "too quiet" complaint; now 100 % (18/18, +33 dB), persisted by WirePlumber in `~/.local/state/wireplumber/default-routes`. Not yet calibrated against his voice (may be hot); if it drifts back down suspect a browser WebRTC AGC (`--disable-features=WebRtcAllowInputVolumeAdjustment` for Brave). Render still applies `loudnorm`.
